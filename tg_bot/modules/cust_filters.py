...
def filters(bot: Bot, update: Update):
    ...
    if len(extracted) >= 2:
        ...
        if not content:
            msg.reply_text("Message composed with only buttons isn't possible.")
            return
...
def stop_filter(bot: Bot, update: Update):
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            update.effective_message.reply_text("Trigger deactivated!")
            raise DispatcherHandlerStop
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        message.reply_text("You seem to be trying to use an unsupported url protocol. Telegram "
                                           ...
                                           "again, or ask at @eduardstal for help.")
                    elif excp.message == "Reply message not found":
                        bot.send_message(chat.id, filt.reply, parse_mode=ParseMode.MARKDOWN,
                                         disable_web_page_preview=True,
                                         reply_markup=keyboard)
                    else:
                        message.reply_text("This note could not be sent, as it is incorrectly formatted. Ask at "
                                           "@eduardstal if you can't figure out why!")
